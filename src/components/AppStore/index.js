...
class AppStore extends Component {
  state = {userInputValue: appsList, userText: '', selectedTab: ''}

  userInput = event => {
    const userValue = event.target.value
    const filteredData = appsList.filter(each =>
      each.appName.toLowerCase().includes(userValue.toLowerCase()),
    )
    this.setState({userInputValue: filteredData, userText: userValue})
  }

  category = category => {
    const {userText} = this.state
    console.log(category)
    const filterCategory = appsList.filter(each => each.category === category)
    const filterCategoryText = filterCategory.filter(each =>
      each.appName.toLowerCase().includes(userText.toLowerCase()),
    )
    this.setState({userInputValue: filterCategoryText, selectedTab: category})
  }

  render() {
    const {userInputValue, userText, selectedTab} = this.state
    return (
      <div className="con">
        <h1>App Store</h1>
        <div className="searchIcon">
          <input type="search" onChange={this.userInput} value={userText} />
          <img
            src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png"
            alt="search icon"
            className="searchIconlogo"
          />
        </div>

        <ul className="listElements">
          {tabsList.map(each => (
            <li className="tabItems" key={each.tabId}>
              <button
                type="button"
                onClick={() => this.category(each.tabId)}
                className={selectedTab === each.tabId ? 'buttonClass' : ''}
              >
                {each.displayText}
              </button>
            </li>
